...
- 💾 **存档不丢** — 自动存本地，刷新自动恢复；「保存 HTML」可导出**自包含的单文件 deck**（运行时内联），双击即开、可再编辑出图、可分享
...
打开它 → 翻页浏览 → 点「导出全部」拿到图片。完整的写法见 [`PROMPT.md`](./PROMPT.md)。
...
2. 描述你的主题，比如「做一张讲『3 个时间管理技巧』的小红书卡，5 页」；
...
| `story` | 9:16 | 1080×1920 | 全屏故事 / 海报 |

## 主题

`light` · `dark` · `warm` · `ink` · `mint` · `gradient` · `ocean` · `sky` · `sunset` · `forest` · `paper` · `bold` · `pastel` · `tech` · `cream` · `night`，写在卡片的 `data-theme` 上。建议整套统一一个主题。想要专属配色，在 section 上内联改 token：`style="--cm-accent:#ff5a5f"`。
